...
class QuantileClusterer(AbstractClusterer):
    ...
    def clusterData(self, current_geography, df_vector=None, df_matrix=None):
        ...
        quantiles_values = np.quantile(df_vector[self.column_to_cluster],
                                       self.quantiles)
        ...
        for idx, q in enumerate(quantiles_values):
            ...
            df_tmp = pd.DataFrame({
                con.ID:
                df_vector[df_vector[self.column_to_cluster].lt(q)][
                    con.ID].values
            })
            ...
            df_vector = df_vector[df_vector[self.column_to_cluster].ge(
                q)].copy()
